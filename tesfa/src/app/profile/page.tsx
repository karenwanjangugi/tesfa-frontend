...
export default function ProfilePage() {
  ...
  if (loading) return <div className="flex justify-center items-center text-center min-h-screen p-4"><p>Loading...</p></div>;
  if (error) return <div className="flex justify-center items-center text-center min-h-screen text-red-500 p-4">{error}</div>;
  ...
  return (
    ...
    <main className="flex w-full flex-col items-center bg-[#FCF6F7] overflow-x-hidden min-h-[100dvh]">
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-10 mx-auto mt-8 mb-6">
        <h1 className="text-3xl sm:text-4xl font-medium text-[#00353D] mb-2">Profile</h1>
            ...
            {profile.logo_image &&
              profile.logo_image.trim() !== "" && (
                <img
                  ...
                  className="object-contain w-[140px] sm:w-[180px] h-[140px] sm:h-[180px]"
                />
              )}
          </div>
         <button
  ...
  <Edit2 className="w-4 sm:w-5 h-4 sm:h-5 text-[#C3A041]" />
        ...
        <div className="w-full max-w-lg bg-[#F3FBFD] rounded-2xl flex flex-col items-center shadow-[12px_12px_32px_rgba(0,0,0,0.15)] px-5 sm:px-7 py-8 sm:py-10 gap-6 sm:gap-8">
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full">
            ...
            <div>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium mb-1">Organization name</p>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium">{profile.org_name}</p>
            </div>
          </div>
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full">
            <Mail className="w-7 sm:w-8 h-7 sm:h-8 text-[#C3A041] mt-0.5" />
            <div>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium mb-1">Email</p>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium break-words">{profile.email}</p>
            </div>
          </div>
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full">
            <Calendar className="w-7 sm:w-8 h-7 sm:h-8 text-[#C3A041] mt-0.5" />
            <div>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium mb-1">Registration Date</p>
              <p className="text-lg sm:text-xl text-[#00353D] font-medium">{formatDate(profile.created_at)}</p>
            </div>
          </div>
        </div>
      </div>
